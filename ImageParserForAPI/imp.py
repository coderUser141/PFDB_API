...
class ImageParser:
	...
	def parse_screenshot(self, fullscreen_image, wtype):
...
if len(sys.argv) == 1:
	...
	COMMAND USAGE: imp [OPTIONS] (TESSBINPATH) FILEPATH TYPE
		
	OPTIONS:
		-h  --help  Displays this help message, and exits
		-c          Tessbin folder exists in the current directory (TESSBIN is ignored)
	...
	TYPES: Specifies the type of weapon to be processed
		...
		if len(sys.argv) == 4:
			...
			print("Too many parameters. Expected 4: imp -c FILEPATH TYPE")
		else:
			print("Too few parameters. Expected 4: imp -c FILEPATH TYPE")
			sys.exit()
	elif sys.argv[1] == "-f":
		if len(sys.argv) == 5:
			pa = ImageParser(sys.argv[2])
			pa.parse_screenshot(sys.argv[3],int(sys.argv[4])) #tessbinpath becomes filepath, filepath becomes type
		elif len(sys.argv) > 5:
			print("Too many parameters. Expected 5: imp -f TESSBINPATH FILEPATH TYPE")
		else:
			print("Too few parameters. Expected 5: imp -f TESSBINPATH FILEPATH TYPE")
			sys.exit()
	else:
		print("i force fed myself olives for a month, i hated it, but now i love olives")
