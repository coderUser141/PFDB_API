...
class PyExec
{
    ...
    //weaponType: 1 = gun, 2 = grenade, 3 = melee
    /// <summary>
    ...
    /// <returns>A Tuple, with the first item containing the result; the second containing the first stopwatch time, and the third containing the second stopwatch time. Note that the second and third items return -1.0 when the function fails.</returns>
    /// <exception cref="ArgumentNullException"></exception>
    /// <exception cref="ArgumentException"></exception>
    public Tuple<string, double, double> exec(string filename, int weaponType, string? tessbinpath)
    {
            ...
            using(Process? execute = Process.Start(pyexecute))
            {
                if(execute != null)
                {
                    using(StreamReader reader = execute.StandardOutput)
                    {
                        ...
                        string endTime = end.ToString("dddd, MMMM, yyyy HH:mm:ss:fff");
    ...
    /// <summary>
    /// Executes the Python Executable asynchronously. List of weaponTypes:
    /// <list type="bullet">
    /// <item>1 = gun</item>
    /// <item>2 = grenade</item>
    /// <item>3 = melee</item>
    ...
    /// <exception cref="ArgumentNullException"></exception>
    /// <exception cref="ArgumentException"></exception>
    public Task<Tuple<string, double, double>> execAsync(string filename, int weaponType, string? tessbinpath)
    {
            ...
            using (Process? execute = Process.Start(pyexecute))
            {
                if (execute != null)
                {
                    using (StreamReader reader = execute.StandardOutput)
                    {
                        ...
                        return Task.FromResult(Tuple.Create(finalOutput, (end - start).TotalSeconds, ((double)watch.ElapsedMilliseconds) / ((double)1000)));
                    }
                }
            }
        }

        return Task.FromResult(Tuple.Create("failed", -1.0, -1.0));
    }
